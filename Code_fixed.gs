const CONFIG = Object.freeze({
  DOMAIN: 'maillys.shop',
  ALLOWED_GMAILS: Object.freeze(['monty746901@gmail.com']),
  MAX_THREADS: 50,
  MAX_MESSAGES: 100,
...
function normalizeAndValidateEmail_(input) {
  ...
  const maillysPattern = new RegExp(`^[a-z0-9._+-]+@${escapedDomain}$`, 'i');
  const isMaillysEmail = maillysPattern.test(email);
  const isAllowedGmail = CONFIG.ALLOWED_GMAILS.includes(email);

  if (email.length > 254 || (!isMaillysEmail && !isAllowedGmail)) {
    throw new Error('อีเมลนี้ไม่ได้รับอนุญาตให้ใช้งาน');
  }
  return email;
}

function isMessageExpired_(message) {
  return Date.now() - message.getDate().getTime() >= CONFIG.MESSAGE_TTL_MS;
}
...
function isAllowedSender_(message, app) {
  ...
  return addresses.some((address) => {
    ...
    return allowedDomains.some((allowed) => senderDomain === allowed || senderDomain.endsWith('.' + allowed));
  });
}
...
function classifyAllowedMessage_(app, subject, body) {
  ...
  const signInPatterns = app === 'disney' ? commonSignInPatterns.concat(disneySignInPatterns) : commonSignInPatterns;
...
function cleanBody_(body) {
  return String(body || '').replace(/\r\n/g, '\n').replace(/\n{4,}/g, '\n\n\n').trim().slice(0, CONFIG.BODY_LIMIT);
}

function prepareHtmlBody_(html, plainBody) {
  let safe = String(html || '').trim();
  if (!safe) safe = '<pre>' + escapeHtml_(plainBody || '(ไม่มีเนื้อหาข้อความ)') + '</pre>';
  safe = safe
  ...
  if (/<head[\s>]/i.test(safe)) return safe.replace(/<head([^>]*)>/i, '<head$1>' + additions);
